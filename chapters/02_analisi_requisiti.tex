...
\noindent \textbf{Attori principali}: Sviluppatore web.\\
\textbf{Precondizioni}: L’utente tenta di avviare l’estensione in una pagina di sistema del \glslink{browserg}{browser} (es. \texttt{chrome://}, \texttt{edge://}, \texttt{about:}, nuova scheda).\\
\textbf{Descrizione}: L’estensione rileva che la pagina corrente appartiene alle aree riservate del \glslink{browserg}{browser}, non accessibili alle estensioni. L'estensione apre un popup informativo che comunica all’utente l’impossibilità di utilizzare \textit{SviluppAbile} in quella pagina.\\
...
\addcontentsline{toc}{paragraph}{UC4: Gestione errori di comunicazione con l’\acrshort{ia}}
\noindent \textbf{Attori principali}: Sviluppatore web.\\
...
\textbf{Descrizione}: L’estensione gestisce risposte non valide o errori di \glslink{parsingg}{parsing} JSON provenienti dal \glslink{serverg}{server} dell’\acrshort{ia} notificando l’utente.\\
...
\hline
\textbf{RO-04} & L’estensione deve impedire l’avvio su pagine di sistema del \glslink{browserg}{browser} (es. chrome://, about:blank, nuova scheda). & UC3\\
\hline
